...
dst %>% 
  ...
  labs(
    x = 'Difficulty',
    y = 'Mean Proportion Choose Safe Deck',
    caption = TeX(r'($N = 131$; Difficulty: $F(1, 118) = 44.58, p < .001, \eta_p^2 = .274)')
  ) +
  ylim(0, 1) +
  scale_fill_manual(name = 'Difference', values = c(Moderate = 'dark grey', Extreme = 'white')) +
  theme_bw() + 
...
```
```{r}
ggsave('difficulty_effect_ppt.pdf', height = 768 / 96, width = 1024 / 96, units = 'in', dpi = 96)
```
...
dst %>% 
  ...
  labs(
    x = 'Difficulty',
    y = 'Proportion Choose Safe Deck',
    caption = 'Red diamonds reflect condition means. Horizontal, black, solid lines reflect medians.'
  ) +
...
```{r}
ggsave('difficulty_effect_id_ppt.png', height = 768 / 96, width = 1024 / 96, units = 'in', dpi = 96)
```
